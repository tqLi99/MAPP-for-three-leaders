...
OBSTACLE_PRESETS = {
    "easy": {
        "static_radius_range": (0.6, 1.0),
        "dynamic_radius_range": (0.6, 0.9),
        "dynamic_speed_range": (0.02, 0.06),
        "min_agent_distance": 3.5,
        "min_target_distance": 3.5,
        "min_obstacle_distance": 0.8,
        "max_attempts": 200,
    },
    "medium": {
        "static_radius_range": (0.7, 1.2),
        "dynamic_radius_range": (0.7, 1.1),
        "dynamic_speed_range": (0.04, 0.10),
    ...
    "hard": {
        "static_radius_range": (0.8, 1.6),
        "dynamic_radius_range": (0.8, 1.4),
        "dynamic_speed_range": (0.08, 0.18),
        "min_agent_distance": 2.5,
        "min_target_distance": 2.5,
...
class AdaptiveFormationEnvironment:
    ...
    def __init__(
        ...
        self.obstacle_config = self._resolve_obstacle_config(difficulty, obstacle_config)
        ...
        # 导航目标点
        self.target_position = np.array([self.world_size * 0.6, self.world_size * 0.9])
    ...
    def _resolve_obstacle_config(self, difficulty, obstacle_config):
        base = OBSTACLE_PRESETS.get(difficulty, OBSTACLE_PRESETS["medium"])
        config = dict(base)
        if obstacle_config:
            config.update(obstacle_config)
        return config
    ...
    def _sample_valid_position(
        ...
        for _ in range(max_attempts):
            pos = np.random.rand(2) * (self.world_size - 2 * radius) + radius

            valid = True
            for obs in existing_obstacles:
                if np.linalg.norm(pos - obs['position']) < (radius + obs['radius'] + min_obstacle_distance):
                    valid = False
                    break
            if not valid:
                continue

            for agent_pos in self.positions:
                if np.linalg.norm(pos - agent_pos) < (radius + min_agent_distance):
                    valid = False
                    break
            if not valid:
                continue

            if np.linalg.norm(pos - self.target_position) < (radius + min_target_distance):
                continue

            if hull is not None and self._obstacle_intersects_convex_hull(pos, radius, hull):
                continue
    ...
    def _initialize_mixed_static_obstacles(self):
        ...
        min_r, max_r = config['static_radius_range']
        ...
        obstacles = []
        hull = self._compute_convex_hull(self.positions) if self.n_agents >= 3 else None

        for _ in range(self.n_static_obstacles):
            radius = float(np.random.uniform(min_r, max_r))
    ...
    def _initialize_mixed_dynamic_obstacles(self):
        config = self.obstacle_config
        min_r, max_r = config['dynamic_radius_range']
        speed_min, speed_max = config['dynamic_speed_range']
        min_agent_distance = config['min_agent_distance']
        min_target_distance = config['min_target_distance']
        min_obstacle_distance = config['min_obstacle_distance']
        max_attempts = config['max_attempts']
        ...
        obstacles = []
        hull = self._compute_convex_hull(self.positions) if self.n_agents >= 3 else None
        existing = list(getattr(self, 'static_obstacles', []))
    ...
    def _point_in_convex_polygon(self, point, polygon, eps=1e-9):
        if len(polygon) < 3:
            return False
    ...
    def _obstacle_intersects_convex_hull(self, center, radius, hull):
        if self._point_in_convex_polygon(center, hull):
            return True
        if radius <= 0:
            return False
    ...
    def _check_obstacles_in_convex_hull(self):
        if self.n_agents < 3:
            return False

        hull = self._compute_convex_hull(self.positions)
        if len(hull) < 3:
            return False

        for obstacle in self.static_obstacles + self.dynamic_obstacles:
            if self._obstacle_intersects_convex_hull(obstacle['position'], obstacle['radius'], hull):
                return True
    ...
    def step(self, actions):
        ...
        hull_violation = self._check_obstacles_in_convex_hull()
        self.hull_violation = hull_violation
        if not collision_occurred and hull_violation:
            collision_occurred = True
            collision_type = 'convex_hull_violation'
            collision_agent = None
...
class ImprovedRewardCalculator:
    ...
    def compute_rewards(self):
        ...
        hull_violation = getattr(self.env, 'hull_violation', False)
        ...
        for i in range(self.env.n_agents):
            ...
            if hull_violation:
                rewards[i] += self.reward_weights['hull_violation']
...
def train_optimized_masac(env_class, n_episodes=2000, max_steps=200, batch_size=128, buffer_size=int(2e6),
    ...
    # 模型存档路径
    os.makedirs('models_01', exist_ok=True)
    ...
    for episode in range(1, n_episodes + 1):
        ...
        for i in range(n_agents):
            ...
            if hasattr(agents[i].alpha, 'detach'):
                alpha_value = agents[i].alpha.detach().item()
            else:
                alpha_value = agents[i].alpha
        ...
        if episode % save_every == 0:
            save_dir = f'models_01/episode_{episode}'
...
if __name__ == "__main__":
    ...
    try:
        env = AdaptiveFormationEnvironment(
            n_agents=3,
            world_size=20,
            n_static_obstacles=10,  # 使用4个静态障碍物（包括固定和随机）
            n_dynamic_obstacles=5  # 使用1个动态障碍物
        )
        ...
        history, agents = train_optimized_masac(
            ...
            n_episodes=10000,  # 适中的训练回合数
            max_steps=200,
            batch_size=256,  # 适中的批次大小以节省内存
